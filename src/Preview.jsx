...
import { Box, X, Share2, Copy, CheckCircle2, Eye, EyeOff } from 'lucide-react';
import { QRCodeCanvas } from 'qrcode.react';
import { Color } from 'three';

const Preview = () => {
  const { setCurrentDesign, setShowLabels } = useStore();
  ...
  }, [setCurrentDesign]); // Satisfies exhaustive-deps
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative', background: '#000' }}>
      ...
      {/* Action Buttons */}
      <div style={{ position: 'absolute', top: '2rem', right: '2rem', zIndex: 100, display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'flex-end' }}>
        <button
          className="btn glass"
          onClick={() => {
            setHideLabels(!hideLabels);
            setShowLabels(!hideLabels);
          }}
          style={{ padding: '0.5rem', borderRadius: '9999px', width: '48px', height: '48px' }}
          title={hideLabels ? "Show labels" : "Hide labels"}
        >
          {hideLabels ? <Eye size={20} /> : <EyeOff size={20} />}
        </button>
        <button
          className="btn glass"
